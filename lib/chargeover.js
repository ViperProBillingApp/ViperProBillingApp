...
export function mapCustomer(c) {
  ...
  return {
    ...
    name: (contact.name || c.company || "").trim(),
    email: (contact.email || c.email || "").trim(),
...
// existing clients get identity fields refreshed while every CRM-managed field
// (segment, stage, tags, notes, amount, activity, reminders…) is preserved.
...
export function mergeCustomers(state, mapped) {
  ...
  for (const m of mapped) {
    ...
    if (i !== -1) {
      ...
      // ponytail: identity refresh only on update — billingStatus/amount stay
      // staff-owned until the ChargeOver mapping is verified on the live account.
      updated++;
    } else {
