// Transactional email via Brevo. Gated on BREVO_API_KEY — brevoSend() always
// returns { ok, messageId }: ok is false (messageId null) if the key is missing
// or Brevo rejects the send, so callers can fall back gracefully. The three
// staff-mail helpers below (sendDigestEmail, sendResetEmail, sendInviteEmail)
// don't need the messageId and unwrap this to a plain boolean.
...
// Weekly (Monday) KPI digest to staff. Staff-facing, so it auto-sends (see PRODUCT.md
// principle 2) — client mail stays review-first.
// Unwraps to a boolean: callers do `if (!ok)`, and an object is always truthy.
...
export async function sendResetEmail(to, name, link) {
  ...
  return (await brevoSend(to, name, "Reset your ViperPro password", `<p>${hi}</p>
...
<p>Best,<br>ViperPro Accounting Team</p>`)).ok;
}
...
export async function sendInviteEmail(to, name, link) {
  const hi = name ? `Hi ${name},` : "Hi,";
  return (await brevoSend(to, name, "Your ViperPro Client Billing CRM login", `<p>${hi}</p>
