...
// itself. `before`/`after` must be in the same representation (both pre- or
// both post-field-encryption) or an unchanged secret will falsely diff — the
// caller passes the already-encrypted `after` (what actually got persisted)
// against `before` (read from stored state, itself already encrypted from a
// prior save), so encStr's idempotent no-op on ciphertext keeps a genuinely
// unchanged secret reading as unchanged. Pure so it's unit-testable without a
// live DB/session, same reason the reply-matching cascade is a pure function.
export function diffClientChanges(upserts, beforeById, afterById) {
  return upserts.map((c) => {
    const before = beforeById.get(c.id);
    const after = afterById.get(c.id);
    if (!after) return null; // upserted then deleted in the same save — nothing to diff
    const keys = before
      ? Object.keys(after).filter((k) => JSON.stringify(after[k]) !== JSON.stringify(before[k]))
      : ["(new client)"];
    return keys.length ? { id: c.id, company: after.company || after.name || "", keys } : null;
  }).filter(Boolean);
}
