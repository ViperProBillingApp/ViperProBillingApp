...
const SCHEMA_SQL = `
  ...
  -- Recycle bin. "Delete permanently" on a client card drops the whole record
  -- here first, so a mis-click is recoverable. Nothing prunes it — deletions are
  -- rare and a stale row costs a few KB, which is cheaper than losing a client.
  ...
  CREATE INDEX IF NOT EXISTS deleted_clients_at_idx ON deleted_clients (deleted_at DESC);
  ...
  -- F-13: inbound/outbound email history for reply detection. Own table, not the
  -- client state blob (see lib/emailstore.js header comment for why).
  ...
  REVOKE ALL ON kv, users, sessions, password_resets, audit_log, rate_hits, state_backups, clients, deleted_clients, tasks, viper_customers, email_messages FROM anon, authenticated;
`;
