...
function key() {
  ...
  // Fail LOUD on a set-but-broken key. Silently disabling encryption would store
  // new secrets in plaintext AND decrypt every existing enc:v1: value to "",
  // inviting staff to overwrite recoverable ciphertext with plaintext.
  if (buf.length !== 32) throw new Error("ENCRYPTION_KEY is set but is not a valid 32-byte key (64 hex chars or base64) — refusing to run with broken encryption");
  return buf;
}
