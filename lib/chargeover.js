...
export const CO_DUPLICATE_IDS = {
  ...
  253: 264, // Travel Excellence S.A. -> Travel Excellence (NB: both carried $250 balances)
  271: 264, // Travel Excellence Costa Rica -> Travel Excellence
...
// Monthly Recurring Revenue depends on client.amount, which is 0 for most
// CSV-imported clients — ChargeOver's /customer list doesn't carry a recurring
// rate, so we infer one from each client's most recent real invoice. That's
// one API call per client, so this only backfills a bounded batch per sync
// (prioritising clients ChargeOver says have an outstanding balance) rather
// than all ~250 at once, to stay well inside the serverless time limit.
...
export async function backfillRecurringAmounts(clients) {
  const candidates = clients
    .filter((c) => c.chargeoverId && !(Number(c.amount) > 0))
    .sort((a, b) => (b.coBalance || 0) - (a.coBalance || 0))
    .slice(0, BACKFILL_LIMIT);
  const start = Date.now();
  let filled = 0;
  await pool(candidates, async (c) => {
    if (Date.now() - start > BACKFILL_BUDGET_MS) return;
    const invoices = await fetchInvoices(c.chargeoverId);
    const latest = invoices.find((i) => i.total > 0 && !i.voided); // void/write-off never sets the rate
    if (latest) { c.amount = latest.total; filled++; }
  }, BACKFILL_CONCURRENCY);
  return { filled, remaining: Math.max(0, clients.filter((c) => c.chargeoverId && !(Number(c.amount) > 0)).length) };
}
