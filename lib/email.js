...
async function brevoSend(to, name, subject, htmlContent) {
  ...
  const r = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({ sender: FROM, to: [{ email: to, name: name || undefined }], subject, htmlContent }),
  });
  return r.ok;
}

// Plain-text client email (Comms tab). Escapes HTML, preserves paragraphs.
export function sendClientEmail(to, name, subject, text) {
  const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const html = esc(String(text)).trim().split(/\n{2,}/).map((p) => `<p>${p.replace(/\n/g, "<br>")}</p>`).join("\n");
