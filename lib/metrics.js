...
export function arrearsPeriods(c, now = new Date()) {
  ...
  if (owed != null && owed <= 0) return 0;
  // Exact when the sync stored it: the count of overdue invoices. Dividing the
  // balance by the rate miscounts any invoice that bundles a one-time charge
  // (a $400 fee + $500 setup on one invoice is ONE period behind, not two).
  if (owed != null && c.coOverdueCount != null) return Math.max(1, Math.min(24, Number(c.coOverdueCount) || 0));
...
export function needsFollowUp(c, now = new Date()) {
  ...
  if (followUpDue(c, now)) return true; // an explicit follow-up date is deliberate — always honour it
  // Offices covered by a group card are chased via the group's master card,
  // exactly like reminders — their stage alone doesn't put them on the list.
  if (coveredByGroup(c)) return false;
  return FOLLOWUP_STAGES.includes(c.stage);
}

// F-09: neutralise spreadsheet formula injection. A cell that starts with
// = + - @ (or tab/CR) is executed as a formula by Excel/Sheets — prefix with a
// single quote so it opens as text. Apply to every string value before unparse.
...
export function csvSafeRow(obj) {
  const out = {};
  for (const k in obj) out[k] = csvSafe(obj[k]);
  return out;
}

// Default USD, not GBP: ChargeOver bills in dollars and a third of the cards
// carry no currency at all, so a GBP fallback printed real $ balances as £.
export function fmtMoney(a, cur = "USD") {
  return `${SYMBOL[cur] || "$"}${(Number(a) || 0).toLocaleString("en-GB", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`;
}

// The predicates behind the StatStrip numbers, pulled out so clicking a metric
// can re-select exactly the clients that metric counted. computeKpis uses these
// too — one definition, so the tile and the drill-down can't drift apart.
export function owesNow(c, now = new Date()) {
  if (coveredByGroup(c)) return false; // billed via the group master
  if (c.coBalance != null) return owedBalance(c) > 0; // live ChargeOver balance wins
  return periodsBehind(c, now) >= 1; // un-synced: fall back to the estimate
}
// Cards whose rate feeds MRR at all (anything not written off).
export function inMrrBase(c) {
  return !["marked-deletion", "never-charged"].includes(c.billingStatus) && c.stage !== "marked-deletion";
}
// …and of those, the ones with a rate actually on file — the "N/M known" figure.
export function contributesMrr(c) { return inMrrBase(c) && Number(c.amount) > 0; }
export function isNotUpToDate(c) { return ["not-up-to-date", "payment-failed"].includes(c.billingStatus); }
...
export function computeKpis(clients, settings = {}, now = new Date()) {
  ...
  for (const c of clients) {
    // USD last resort: the browser passes settings.currency ("USD" by default),
    // but the cron reads settings straight from the DB where the key is unset.
    const cur = c.currency || settings.currency || "USD";
    ...
    if (coveredByGroup(c)) {
      ...
      synced++;
      if (owesNow(c, now)) { overdue++; owedByCur[cur] = (owedByCur[cur] || 0) + owedBalance(c); }
    } else if (owesNow(c, now)) {
      overdue++; owedByCur[cur] = (owedByCur[cur] || 0) + periodsBehind(c, now) * (Number(c.amount) || 0);
    }
    if (inMrrBase(c)) {
      mrr += monthlyValue(c);
      if (contributesMrr(c)) { mrrKnown++; active++; }
    }
    if (isNotUpToDate(c)) notUpToDate++;
