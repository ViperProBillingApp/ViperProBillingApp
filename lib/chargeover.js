...
export function mapCustomer(c) {
  ...
  return {
    ...
    // "i" = customer_status_name "Inactive" (customer_status_str "inactive-manual")
    // — ChargeOver's own record of this account being closed/deactivated, distinct
    // from CO_DUPLICATE_IDS (which is about two records for one company). Confirmed
    // 2026-09-03: customer 60 (Grupos Incentivos Terramar) sat inactive here while
    // its duplicate 141 was the real active account with a real subscription.
    inactive: c.customer_status_state === "i",
  };
}
...
// Per-customer overdue picture: total overdue balance AND how many overdue
// invoices make it up. The count is what "periods behind" really means — a
// $900 invoice ($400 fee + $500 one-time setup) is ONE period, not two, which
// balance ÷ rate gets wrong.
export async function fetchOverdueMap() {
  ...
  for (let offset = 0; offset < 5000; offset += limit) {
    ...
    for (const i of list) {
      ...
      const e = (map[id] ||= { amount: 0, count: 0 });
      e.amount += Number(i.balance) || 0;
      e.count += 1;
    }
...
export const CO_DUPLICATE_IDS = {
  ...
  60: 141,  // Grupos Incentivos Terramar -> GIT tax-id name (Darryl-confirmed 2026-09-03: 141 is the active account in ChargeOver, 60 is marked inactive there — opposite of what this entry originally assumed)
  96: 268,  // Summit Events LLC -> Summit Events
};

// Customers retired from the CRM on purpose. Deleting a ChargeOver-linked client
// does NOT stick on its own: the next sync sees the customer in ChargeOver, finds
// no matching card, and re-adds it — as a HOLLOW record, because ChargeOver's
// /customer endpoint carries no email or contact. That is exactly how five cards
// silently lost their contact details on 2026-07-28. Listing the id here makes the
// deletion permanent. Their invoice history stays untouched in ChargeOver.
export const CO_RETIRED_IDS = new Set(["69", "121", "175", "257", "261",
  "212", // 3T Ventures LLC — no longer a customer; deleted twice, resurrected by the sync each time
]);

// Two CRM cards accidentally carrying the same ChargeOver ID is a real,
// recurring failure mode — not the CO_DUPLICATE_IDS case above (which is
// ChargeOver itself having two customer records for one company). This is
// our own bug: a group master card gets created for an office that already
// had a synced card, or a hollow "Imported from ChargeOver" card gets added
// alongside one a human already created manually. findIdx() below only ever
// updates the FIRST card it finds with a given ID, so the other one silently
// freezes at its last-known balance — sometimes for months, since nothing
// about it looks broken until someone happens to compare it to the real
// card (that's how RMC, PRA, Emeco, Lafayette, Hello!, IMI, Sunlinc, and
// Spaintacular all went stale on 2026-08 before being found and fixed).
// This is a pure detector — call it after every merge and surface what it
// finds loudly (sync response, digest email); it never changes any data.
export function findDuplicateChargeoverIds(clients) {
  const byId = new Map();
  for (const c of clients || []) {
    const id = String(c.chargeoverId || "").trim();
    if (!id) continue;
    if (!byId.has(id)) byId.set(id, []);
    byId.get(id).push({ id: c.id, company: c.company || c.name, archived: !!c.archivedClient });
  }
  const dupes = [];
  for (const [chargeoverId, cards] of byId) {
    if (cards.length > 1) dupes.push({ chargeoverId, cards });
  }
  return dupes;
}
...
export function mergeCustomers(state, mapped, overdueByCo = null) {
  ...
  for (const m of mapped) {
    ...
    if (CO_RETIRED_IDS.has(String(m.chargeoverId))) continue;     // retired on purpose — never re-create it
    if (m.inactive) continue;                                     // ChargeOver marks this account inactive — don't let it drive (or resurrect) a card
    // owed RIGHT NOW: overdue-only when we have the map (raw balance includes
    // pre-generated invoices that aren't due yet)
    const od = overdueByCo ? overdueByCo[m.chargeoverId] : null;
    const owedNow = overdueByCo ? (od?.amount || 0) : m.coBalance;
    const owedCount = overdueByCo ? (od?.count || 0) : null;
    const i = findIdx(clients, m);
    if (i !== -1) {
      const ex = clients[i];
      if (m.chargeoverId) ex.chargeoverId = m.chargeoverId;
      // Company name is staff-curated (merges, renames, disambiguating group
      // offices) and deliberately diverges from ChargeOver's own customer-name
      // field — only fill it in when blank. Unconditionally overwriting it here
      // silently reverted a manual rename on the very next nightly sync with no
      // activity log entry, which is how it went unnoticed for weeks.
      if (m.company && !ex.company) ex.company = m.company;
      ...
      if (overdueByCo) {
        ...
        ex.coOverdueCount = owedCount; // number of overdue invoices — the true "periods behind"
      ...
      clients.push({ ...newClient(m), ...(overdueByCo ? { coOverdue: owedNow, coOverdueCount: owedCount } : {}), billingStatus: owedNow > 0 ? "not-up-to-date" : "current-pricing" });
      added++;
    }
  }
...
// ChargeOver stores a package's price as a monthly figure rounded to whole
// cents, so an annual price never round-trips: $400/yr comes back as
// mrr 33.33 / arr 399.96, $650/yr as 649.92, $2000/yr as 2000.04. The lost
// fraction is under a cent a month, so the yearly figure is out by at most 12c
// either way — snap back to the whole dollar when we're inside that window.
//
// Deliberately NOT read off the line's tierset, which looks like the price but
// is the item's CURRENT catalogue rate: customers on older pricing (34 of them
// on $250/yr) carry a tierset that already says $400, so pricing off it would
// silently re-rate them and inflate MRR.
// ponytail: every price in this book is a whole dollar. A genuine $399.50 would
// be left alone (it lands 52c off), but a genuine $399.95 would snap to $400.
export function unround(cycleAmount) {
  const whole = Math.round(cycleAmount);
  return Math.abs(cycleAmount - whole) <= 0.12 ? whole : round2(cycleAmount);
}
...
export async function fetchRecurring(customerId) {
  ...
  const perCycle = live.map((p) => unround(Number(p.paycycle === "yrl" ? p.arr : p.mrr) || 0));

  const mrr = round2(live.reduce((n, p, i) => n + (p.paycycle === "yrl" ? perCycle[i] / 12 : perCycle[i]), 0));
  // earliest upcoming invoice across the active packages, with its cycle amount
  const next = live
    .map((p, i) => ({ date: String(p.next_invoice_datetime || "").slice(0, 10), amount: round2(perCycle[i]) }))
    .filter((n) => n.date)
    .sort((a, b) => a.date.localeCompare(b.date))[0] || null;

  // What one billing cycle actually costs: the yearly figure for annual
  // clients, the monthly one for everyone else.
  const amount = round2(annual ? perCycle.reduce((n, v) => n + v, 0) : mrr);
  return { mrr, annual, packages: live.length, amount, next };
}
...
export async function backfillRecurringAmounts(clients) {
  ...
  await pool(candidates, async (c) => {
    ...
    c.coAmountAt = stamp;
    c.coHasSubscription = r.packages > 0; // drives the Clients tab's Subscription column
    if (r.packages > 0) {
      c.cadence = r.annual ? "annual" : "monthly";
      c.amount = r.amount; // true cycle price from the tiersets, not mrr x 12
    } else {
