...
// Immediate push for a handful of contacts (e.g. the sender of a reply that
// just arrived) instead of waiting for that email to come up in the nightly
// rotation. `emails` are matched case-sensitively against buildContactSync's
// output, which already lowercases every address.
export async function pushContactsNow(clients, emails) {
  const wanted = new Set((emails || []).map((e) => String(e).trim().toLowerCase()));
  if (!wanted.size) return { pushed: 0, failed: 0 };
  const rows = buildContactSync(clients).filter((r) => wanted.has(r.email));
  let pushed = 0, failed = 0;
