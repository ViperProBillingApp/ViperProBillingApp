...
export async function GET(req) {
  ...
  return NextResponse.json({ visible_password: me.visible_password ? decStr(me.visible_password) : null });
}
...
export async function POST(req) {
  ...
  await db.query("UPDATE users SET hash = $1, visible_password = $2 WHERE id = $3", [hashPassword(String(next)), encStr(String(next)), me.id]);
