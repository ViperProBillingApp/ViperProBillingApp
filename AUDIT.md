...
| F-03 | No immutable audit trail | High | **Fixed for admin/security actions, was NOT for ordinary client edits until 2026-08-05** — the append-only `audit_log` covered login, user management, MFA, credential reveals, and recycle-bin restore/purge, but `PUT /api/clients/batch` (the one route behind every client-card save) only wrote to it for deletes ≥5. A billing-status change, a pricing edit, or a new portal password went nowhere but the client's own mutable `activity` array — exactly the architecture this finding originally criticized. Fixed 2026-08-05: every save now writes a `clients.saved` audit row via `diffClientChanges()` (`lib/clients.js`), naming which top-level keys changed per client — never the values, so a secret write shows as `"portalPassword changed"`, not the secret. Verified with a live-DB round-trip (throwaway client, cleaned up) and `scripts/check-audit-diff.mjs` (`npm run check`) |
